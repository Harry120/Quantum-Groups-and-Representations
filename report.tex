...
% As $SO(3)$ is a Lie group, we get an induced Lie algebra representation $\pi' : \so(3) \to \mathfrak{u}(\H)$. Recalling that $\mathfrak{u}()$ consists of skew adjoint matrices, we see that $X = i\pi'(x)$ is a hermitian operator on $\H$ for any $x \in \so(3)$. Observables generated by $SO(3)$ in this way are known as angular momentum operators, and correspond to the classical .

% Our aim now is to find the eigenvectors and eigenvalues of every $X$. Once this is done, it will be simple to describe the state of a particle in terms of these eigenstates, allowing us to assign a probability to each of the eigenvalues of $iX$. We refer to the eigenvalue of $iX$ as the angular momentum in the direction represented by $X$.

% In view of the Peter Weyl theorem, the compactness of $SO(3)$ implies that $\H$ decomposes as a direct sum of finite dimensional vector spaces $V_l$. These are irreducible under $SO(3)$ and $\so(3)$. As there is exactly one irreducible representation of $\so(3)$ for each odd dimension, we know that 
% $$\H = V_1 \oplus V_3 \oplus V_5 \oplus \hdots.$$
% Therefore, our aim is to find the action of $iX$ on each $V_l$.
...
% It turns out that in a spherically symmetric potential, the Hamiltonian operator $H$ commutes with the action of $L_1, L_2, L_3$, so we see that each of the spaces $V_k$ are also energy eigenspaces. As such, we can label these spaces by their energy. As a `pure' state in quantum mechanics corresponds to an eigenstate of the observables under consideration, we see that there are $2n+ 1$ possible values of the orbital angular momentum at energy level $n$.
